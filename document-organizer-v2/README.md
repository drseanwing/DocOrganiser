...
2. **Mounts** extracted files as a Docker volume for local processing
3. **Analyzes** content with Ollama (local, no API limits)
4. **Detects duplicates** via content hashing + LLM decision-making
5. **Identifies versions** of documents and applies version control logic
6. **Reorganizes** files locally with full rollback capability
7. **Re-zips** and uploads back when approved
...
2. **Manifest Tracking**: Complete audit trail in `_organization_manifest.json`
3. **Review Phase**: Optional human approval before execution
4. **Rollback Data**: All original paths stored for potential restoration
5. **Shortcut Metadata**: Original file info preserved in shortcut records
...
3. Build Index Agent
4. Build Dedup Agent
5. Build Version Agent
6. Build Organization Agent
7. Create n8n workflows for download/upload
